...
plotheight = 600;  % Height of figure (by default in pixels)
...
for i = 1:5
    ...
    ind = find(tdata <= min(tdata) + i*range(tdata)/5);
    
    V = eigVecs(:,:,ind(end));
    D = diag(eigVals(:, ind(end))/(freq^2));
    [x,y] = eig2ellipse(V,D);
    plot(x,y,'k');
    axis equal;
    
    titleStr = strcat('t = ',num2str(tdata(ind(end))));
    title(titleStr);
